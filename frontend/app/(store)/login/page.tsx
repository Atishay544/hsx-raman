      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="w-full max-w-sm">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
            ...
            <div className="flex bg-gray-100 rounded-xl p-1 mb-6 gap-1">
              {tabs.map(t => (
                <button key={t.key} onClick={() => switchTab(t.key)}
                  className={`flex-1 py-1.5 text-xs rounded-lg font-medium transition ${tab === t.key
            ...
            {/* ── Email + Password ── */}
            {tab === 'password' && (
              <form onSubmit={e => { e.preventDefault(); handlePasswordAuth() }} className="space-y-3">
            ...
            {tab === 'email' && step === 'input' && (
              <form onSubmit={e => { e.preventDefault(); handleEmailOtp() }} className="space-y-3">
                <input type="email" placeholder="Email address" value={email}
                  onChange={e => setEmail(e.target.value)} className={inputCls} />
                <div className="flex justify-center">
                  <div className="cf-turnstile" data-sitekey={process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY} data-callback="onTurnstileVerify" />
                </div>
                <button type="submit" disabled={loading || !email || !turnstileToken}
                  className={btnPrimary}>
            ...
            {tab === 'email' && step === 'otp' && (
              <form onSubmit={e => { e.preventDefault(); handleEmailVerify() }} className="space-y-3">
                ...
                <button type="button" onClick={() => setStep('input')} className="w-full text-xs text-gray-400 hover:text-gray-600 transition">
                  ← Change email
                </button>
              </form>
            )}
            ...
            {tab === 'phone' && step === 'input' && (
              <form onSubmit={e => { e.preventDefault(); handlePhoneOtp() }} className="space-y-3">
                ...
                <button type="submit" disabled={loading || phone.length < 10 || !turnstileToken}
                  className={btnPrimary}>
            ...
            {tab === 'phone' && step === 'otp' && (
              <form onSubmit={e => { e.preventDefault(); handlePhoneVerify() }} className="space-y-3">
